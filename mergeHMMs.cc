...
int main( int argc, char **argv ) {
	...
	float curr_weight = 1;			// the current weight for the current model
...
//	new_hmm->printHMM();
	...
	new_hmm->setPi( tmp_new_pi );
	new_hmm->setA( tmp_new_A );
	new_hmm->setB( tmp_new_B );
	curr_hmm->printHMM();
	new_hmm->printHMM();
